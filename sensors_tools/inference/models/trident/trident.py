...
class Trident(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.clip = create_model(
            clip_model_type, pretrained=clip_type, precision="fp16"
        )
        ...
        if sam_model_type != "vit_h" or not sam_refinement:
            ...
            self.sam = (
                sam_model_registry[sam_model_type](checkpoint=sam_ckpt)
                .to(device=device)
                .eval()
                .half()
            )
    ...
    def get_sam_feat(self, tmp_img, stride):
        ...
        sam_valid_h, sam_valid_w = (
            sam_r * self.sam_predictor.input_size[0] // stride,
            sam_r * self.sam_predictor.input_size[1] // stride,
        )  # get the feature shape in SAM Encoder
        sam_valid_h, sam_valid_w = int(sam_valid_h), int(sam_valid_w)
        sam_enc_feats = self.sam_predictor.features  # [1, 256, 64, 64]
        sam_enc_feats = sam_enc_feats[
            :, :, :sam_valid_h, :sam_valid_w
        ]  # [1, 256, new_h, new_w]
        sam_hw = int(sam_r * 64)
        sam_attn = (
            self.sam_predictor.model.image_encoder.last_attn
        )  # [1, 12, 64*64, 64*64]
        sam_attn = sam_attn.view(1, self.sam_heads, sam_hw, sam_hw, sam_hw, sam_hw)[
            :, :, :sam_valid_h, :sam_valid_w, :sam_valid_h, :sam_valid_w
        ]  # [1, 12, new_h, new_w, new_h, new_w]
        sam_attn = sam_attn.flatten(2, 3).flatten(
            3, 4
        )  # [1, 12, new_h*new_w, new_h*new_w]
        sam_v = self.sam_predictor.model.image_encoder.last_v
        sam_v = sam_v[
            :, :, :sam_valid_h, :sam_valid_w, :
        ]  # [1, head, new_h, new_w, head_dim]
    ...
    def get_trident_seg(self, img_pil, img_tensor, img_metas):
        ...
        if self.dataset_type == "CityscapesDataset":
            ...
            feats_1, sam_attn_1, sam_v_1, sam_valid_h_1, sam_valid_w_1 = (
                self.get_sam_feat(img_cv2_1, 16)
            )
            feats_2, sam_attn_2, sam_v_2, sam_valid_h_2, sam_valid_w_2 = (
                self.get_sam_feat(img_cv2_2, 16)
            )
            ...
            sam_enc_feats, sam_attn, sam_v, sam_valid_h, sam_valid_w = (
                self.get_sam_feat(img_cv2, 16)
            )
        ...
        img_batch, paddings, patch_locs, win_sizes = self.get_windowed_imgs(
            processed_img, stride
        )

        imgs_norm = [
            self.norm(self.unnorm(img_batch[i])) for i in range(len(img_batch))
        ]  # replace norm here
        ...
        vfm_feats = (
            feat[:, 1:, :].reshape(nb_im, vfm_h, vfm_w, -1).permute(0, 3, 1, 2)
        )  # batch, c, h, w
        ...
        logits = logits.permute(0, 2, 1).reshape(
            -1, logits.shape[-1], sam_valid_h, sam_valid_w
        )  # B, C, H, W
        logits = F.interpolate(
            logits, size=img_metas[0]["ori_shape"][:2], mode="bilinear"
        )
        return logits.float()
    ...
    def predict(self, img_pil, img_tensor):
        ...
        if self.pamr:
            ...
            img_tensor = nn.functional.interpolate(
                img_tensor, size=img_size, mode="bilinear", align_corners=False
            )
            try:
                seg_logits = self.pamr(img_tensor, seg_logits.to(img_tensor.dtype)).to(
                    self.dtype
                )
