...
def split_connected_regions(
    ...
    for c in range(C):
        ...
        for region_label in range(1, num_regions + 1):
            ...
            region_logit = class_logit * torch.tensor(region_mask).to(
                class_logit.device
            )
...
def map_refinement_coarse(
    refined_masks, refined_logits, coarse_boxes, seg_logits, img_shape
):
    refined_whole_logits = torch.zeros(
        [len(coarse_boxes), img_shape[0], img_shape[1]], device=refined_masks.device
    )  # C, H, W
    cumsum = 0
    for i in range(len(coarse_boxes)):
        ...
        tmp_cls_mask = (
            refined_masks[cumsum : cumsum + tmp_cls_num, 0].sum(dim=0).bool()
        )  # H, W
        tmp_cls_logit = (
            refined_logits[cumsum : cumsum + tmp_cls_num, 0] * seg_logits[i]
        ).max(
            dim=0
        )  # H, W
...
def map_failed_regions(
    refined_masks, refined_logits, failed_regions, seg_logits, segmentation
):
    ...
    refined_logits = torch.where(
        failed_logit > refined_logits, failed_logit, refined_logits
    )
    refined_masks = torch.where(
        failed_logit.max(dim=0, keepdim=True)[0]
        > refined_logits.max(dim=0, keepdim=True)[0],
        segmentation,
        refined_masks,
    )
...
def sam_refinement(
    ...
    if down_ratio != 1:
        segmentations_down = (
            F.interpolate(segmentations.unsqueeze(0).half(), [dh, dw], mode="nearest")
            .squeeze(0)
            .long()
        )
        seg_logits_down = F.interpolate(
            seg_logits.unsqueeze(0), [dh, dw], mode="bilinear", align_corners=False
        ).squeeze(0)
    else:
    ...
    if coarse_thresh > 0:
        segmentations_down[seg_logits_down.max(0, keepdim=True)[0] < coarse_thresh] = (
            num_classes
        )
        cls_pred = (
            F.one_hot(segmentations_down.squeeze(0).long(), num_classes=num_classes + 1)
            .permute(2, 0, 1)
            .float()
        )  # [C, H, W]
        split_last = False
        ...
        cls_pred = (
            F.one_hot(segmentations_down.squeeze(0).long(), num_classes=num_classes)
            .permute(2, 0, 1)
            .float()
        )
        split_last = True

    coarse_regions, coarse_boxes, coarse_boxes_score, coarse_points = (
    ...
    for i in range(num_classes):
        ...
        new_h, new_w = predictor.transform.get_preprocess_shape(
            img.shape[0], img.shape[1], 256
        )
        cls_wise_r = F.interpolate(
            cls_wise_r.unsqueeze(0),
            [new_h, new_w],
            mode="bilinear",
            align_corners=False,
        ).squeeze(0)
        # pad to 256x256
        cls_wise_r = F.pad(
            cls_wise_r, (0, 256 - new_w, 0, 256 - new_h), mode="constant", value=0
        )
        cls_wise_r = torch.where(
            cls_wise_r > coarse_thresh,
            sam_mask_coff * cls_wise_r,
            torch.tensor(0.0).type_as(segmentations),
        ).float()
        cls_wise_b = predictor.transform.apply_boxes_torch(
            tmp_b.type_as(segmentations), [dh, dw]
        ).float()
        cls_wise_p = torch.stack(tmp_p).type_as(segmentations)
        cls_wise_p = predictor.transform.apply_coords_torch(
            cls_wise_p, [dh, dw]
        ).unsqueeze(1)
        cls_wise_mask, cls_wise_score, cls_wise_logit = predictor.predict_torch(
            point_coords=cls_wise_p.float(),  # input_point,
            point_labels=torch.ones([cls_wise_p.shape[0], 1])
            .type_as(segmentations)
            .float(),  # input_label,
            boxes=cls_wise_b,
    ...
    masks, scores, logits = (
        torch.cat(cls_wise_masks, dim=0),
        torch.cat(cls_wise_scores, dim=0),
        torch.cat(cls_wise_logits, dim=0),
    )
    logits = predictor.model.postprocess_masks(
        logits, predictor.input_size, predictor.original_size
    )

    combined_boxes = combined_boxes * down_ratio
    if logits.shape[-1] != img.shape[1] or logits.shape[-2] != img.shape[0]:
        logits = F.interpolate(
            logits, img.shape[:2], mode="bilinear", align_corners=False
        )
        masks = logits > predictor.model.mask_threshold

    refined_masks, refined_logits = map_refinement_coarse(
        masks,
        logits.sigmoid(),
        coarse_boxes,
        seg_logits,
    ...
    refined_masks = torch.where(
        refined_logits.sum(dim=0, keepdim=True) == 0, segmentations, refined_masks
    )
    if len(failed_regions):
        ...
        if down_ratio != 1:
            failed_regions = (
                F.interpolate(
                    failed_regions.unsqueeze(0).float(), img.shape[:2], mode="nearest"
                )
                .squeeze(0)
                .type_as(segmentations)
            )
        refined_masks, refined_logits = map_failed_regions(
            refined_masks, refined_logits, failed_regions, seg_logits, segmentations
        )
